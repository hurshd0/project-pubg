...
def get_data():
    ...
    platforms = ['steam', 'psn', 'xbox', 'kakao']
    for platform in platforms:
        print(f'-------------------- {platform} --------------------')
        matches = get_match_samples(platform=platform)
        print(matches.head())
        df = get_match_stats(matches)
        print(df.head())
        date = datetime.now().strftime('%Y-%m-%d_%I%p')
        save_data_frame(df, f'pubg_stats_{platform}_{date}.csv')
